...
namespace WinCecScreensaverCore;

public partial class MainForm : Form
{
    public readonly Guid _guidConsoleDisplayState = new("6fe69556-704a-47a0-8f24-c28d936fda47");
    ...
    public event DisplayStateChangedHandler? OnDisplayStateChanged;
    ...
    public MainForm()
    {
        ...
        standByButton.Click += (s, e) => OnDisplayStateChanged?.Invoke(false);
        Controls.Add(standByButton);

        var powerOnButton = new Button { Text = "&Power On", Top = standByButton.Height };
        powerOnButton.Click += (s, e) => OnDisplayStateChanged?.Invoke(true);
    ...
    protected override void WndProc(ref Message message)
    {
        if (message.Msg == _wmPowerBroadcast && message.WParam.ToInt32() == _pbtPowerSettingsChange)
        {
            ...
            if (settings.PowerSetting == _guidConsoleDisplayState)
            {
                ...
                {
                    case 0: OnDisplayStateChanged?.Invoke(false); break;
                    case 1: OnDisplayStateChanged?.Invoke(true); break;
                }
            }
        }
